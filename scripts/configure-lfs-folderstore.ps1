...
if ([string]::IsNullOrWhiteSpace($StoreRoot)) {
    if (Test-Path -LiteralPath $defaultStoreRoot) {
        $StoreRoot = $defaultStoreRoot
    } else {
        throw 'Defina GITHUB_LFS_ROOT o use -StoreRoot con la raíz de Github-LFS.'
    }
}

$StoreRoot = [IO.Path]::GetFullPath($StoreRoot)
$repoRoot = (Resolve-Path -LiteralPath (Join-Path $PSScriptRoot '..')).Path
$repoStorage = Join-Path $StoreRoot 'repositories\Tesis-datos'

if ([string]::IsNullOrWhiteSpace($AdapterPath)) {
    $command = Get-Command lfs-folderstore -ErrorAction SilentlyContinue
    if ($command) {
        $AdapterPath = $command.Source
    } else {
        $AdapterPath = Join-Path $StoreRoot 'tools\lfs-folderstore-v1.0.1\lfs-folderstore-windows-amd64\lfs-folderstore.exe'
    }
}

if (-not (Test-Path -LiteralPath $AdapterPath -PathType Leaf)) {
    throw "No se encontró lfs-folderstore en: $AdapterPath"
}
...
Write-Output "Git LFS configurado para Tesis-datos."
Write-Output "Almacén: $repoStorage"
Write-Output "Adaptador: $AdapterPath"
